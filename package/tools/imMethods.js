export default {
  ...
  getGroup(e) {
    let grouplist =
      window.localStorage.getItem("groupList") !== "undefined"
        ? JSON.parse(window.localStorage.getItem("groupList"))
        : [];
    return grouplist[e];
  },
  getRemark(e) {
  ...
  getDocumentSize(content, name) {
    let size =
      this.getUrlParam(content, name) / 1024 > 1024
        ? this.formatFlow(this.getUrlParam(content, name) / 1024 / 1024, 1) +
          "MB"
        : this.formatFlow(this.getUrlParam(content, name) / 1024, 1) > 1
        ? +this.formatFlow(this.getUrlParam(content, name) / 1024, 1) + "KB"
    ...
    for (let i = 0; i < e.length; i++) {
      try {
        ...
        if (type) {
          ...
          if (item.userName)
            e[i] = `<span class=atspan>@${item.userName}</span>`;
        }
      } catch (error) {}
    }
    if (type && status) {
      for (let i = 0; i < e.length; i++) {
        if (typeof e[i] == "string") {
          if (e[i].includes('{"all":true}')) {
            e[i] = e[i].replace(
              '{"all":true}',
              `<span class=atspan>@所有人</span>&nbsp`
            );
          }
          if (e[i].includes('{"all":"true"}')) {
            e[i] = e[i].replace(
    ...
    var set = setInterval(() => {
      if (img.width > 0 || img.height > 0) {
        ...
        if (ys_w > width || ys_h > height) {
          let scale;
          let scale1 = ys_w / width;
          let scale2 = ys_h / height;
          if (scale1 > scale2) {
            scale = scale1;
          } else {
            scale = scale2;
